...
const Login = () => {
  ...
  useEffect(() => {
    if (authState?.isSuccess === true) {
      if (authState?.user?.data === undefined) {
        navigation.navigate("Login");
        ToastAndroid.show("Correo o contraseñan incorrecta", ToastAndroid.SHORT);
      } else {
        navigation.navigate("Home");
        AsyncStorage.setItem("userInfo", JSON.stringify(authState?.user?.data));
        AsyncStorage.setItem("token", JSON.stringify(authState?.user?.token));
        ToastAndroid.show("Bienvenido a la tienda", ToastAndroid.SHORT);
      }
    } 
